function OnWorldTick(World)
	...
	if isStopped[thisWorld] then
		local now = World:GetTimeOfDay()
		if savedTime[thisWorld] == nil then
			savedTime[thisWorld] = now 
		end
		if now >= savedTime[thisWorld] + 400 then
			World:SetTimeOfDay(savedTime[thisWorld])
